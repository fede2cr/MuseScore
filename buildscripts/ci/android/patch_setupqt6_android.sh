...
helper_block = (
    ...
    '\n'
    '#ifdef Q_OS_ANDROID\n'
    '// Resolve the human-readable display name (with extension) for a content://\n'
    '// URI. QFileInfo::fileName() sometimes yields an extension-less document id\n'
    '// (e.g. .../document/123); fall back to querying the ContentResolver for\n'
    '// _display_name so MuseScore can detect the file type by extension.\n'
    'static QString resolveAndroidDisplayName(const QString& uri)\n'
    '{\n'
    ...
    '    }\n'
    '    QString displayName = resolveAndroidDisplayName(uri);\n'
    ...
    '        LOGE() << "Failed to open content uri: " << uri.toStdString();\n'
    '        return muse::io::path_t();\n'
    '    }\n'
    '    if (!dst.open(QIODevice::WriteOnly | QIODevice::Truncate)) {\n'
    '        LOGE() << "Failed to create cache copy: " << cachePath.toStdString();\n'
    '        return muse::io::path_t();\n'
    '    }\n'
    '    const QByteArray data = src.readAll();\n'
    '    const qint64 written = dst.write(data);\n'
    '    if (written != data.size()) {\n'
    '        LOGE() << "Short write copying to cache: " << written << " of " << data.size();\n'
    '        return muse::io::path_t();\n'
    '    }\n'
    '    LOGI() << "Copied picked file to cache (" << written << " bytes): " << cachePath.toStdString();\n'
